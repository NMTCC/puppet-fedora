...
class profiles::nmt {
  ...
  profiles::nmt::execnot { 'kinit': command => '/bin/kinit -k', onlyifnot => '/bin/kinit -k', }
  ...
  profiles::nmt::execnot { 'yum-makecache': command => '/bin/yum makecache', timeout => 0, onlyifnot => '/bin/yum makecache', } ->
  profiles::nmt::execnot { 'yum-update': command => '/bin/yum update -y --setopt=protected_multilib=false', timeout => 0, onlyifnot => '/bin/yum check-update', }
